...
$sidebar_name = isset($user['nom']) ? $user['nom'] : ($_SESSION['user_nom'] ?? ($_SESSION['user_name'] ?? 'Gestionnaire Bat A'));
$sidebar_avatar = isset($user['avatar']) ? $user['avatar'] : ($_SESSION['user_avatar'] ?? '');
if (empty($sidebar_name)) $sidebar_name = 'Gestionnaire Bat A';
...
function render_avatar($avatar, $name) {
    if (!empty($avatar)) {
        ...
        $initials = isset($vals[1])
                ? strtoupper(substr($vals[0], 0, 1) . substr($vals[1], 0, 1))
                : strtoupper(substr($vals[0], 0, 2));
...
<div class="d-md-none w-100 bg-white shadow-sm p-3 mb-3 d-flex justify-content-between align-items-center">
    ...
    <button class="btn btn-light" type="button" data-bs-toggle="offcanvas" data-bs-target="#gestionnaireSidebar">
        <i class="fas fa-bars fa-lg text-dark"></i>
    </button>
</div>

<div class="offcanvas offcanvas-start" tabindex="-1" id="gestionnaireSidebar" style="background-color: var(--sidebar-bg); color: white;">
    <div class="offcanvas-header">
        <h5 class="offcanvas-title text-white">Menu Gestionnaire</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="offcanvas"></button>
    </div>
    <div class="offcanvas-body">
        <div class="user-profile-card d-flex align-items-center mb-4 p-3" style="background: rgba(255, 255, 255, 0.1); border-radius: 12px;">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item">
                <a href="reclamations.php" class="nav-link <?php echo ($current_page == 'reclamations.php') ? 'active' : ''; ?>">
                    <i class="fas fa-list-check"></i> Réclamations
                </a>
            </li>
            <li class="nav-item">
                <a href="annonces.php" class="nav-link <?php echo ($current_page == 'annonces.php') ? 'active' : ''; ?>">
...
<div class="col-md-3 col-lg-2 sidebar d-none d-md-block px-0">
    ...
    <div class="user-profile-card d-flex align-items-center" style="margin: 0 20px 40px 20px; padding: 10px 15px; background: rgba(255, 255, 255, 0.1); border-radius: 12px;">
        ...
        <div>
            <div class="fw-bold lh-1"><?php echo htmlspecialchars($sidebar_name); ?></div>
            <small class="text-white-50" style="font-size: 12px;">Gestionnaire</small>
        </div>
    </div>
